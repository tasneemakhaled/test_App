...
Future<void> handleSignUp(
  ...
  try {
    ...
    final registerModel = RegisterModel(
      ...
      doctorLicense: selectedRole == 'DOCTOR' ? doctorLicenseController.text.trim() : null,
    );
    ...
    print("🔹 Register Response: token=${response.token}, role=${response.role}");
    ...
    if (response.token == "temp_token") {
      ...
      try {
        ...
        print("🔹 Login Response: token=${loginResponse.token}, role=${loginResponse.role}");
        ...
        if (loginResponse.role == 'DOCTOR') {
          ...
          await Future.delayed(Duration(milliseconds: 500));  // تأخير صغير للتأكد من أن SnackBar مرئي
    ...
    if (response.role == 'DOCTOR') {
      print("🔷 Navigating to Doctor Home View");
      await Future.delayed(Duration(milliseconds: 500));  // Small delay to ensure SnackBar is visible
      Navigator.pushAndRemoveUntil(
        context,
        MaterialPageRoute(builder: (context) => const DoctorHomeView()),
        (route) => false,
